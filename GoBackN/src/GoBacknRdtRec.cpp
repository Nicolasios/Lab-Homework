#include "Global.h"
#include "GoBacknRdtRec.h"
#include "DataStructure.h"
GoBacknRdtRec::GoBacknRdtRec() : expectSequenceNumberRcvd(1)
...
GoBacknRdtRec::~GoBacknRdtRec()
{
}

void GoBacknRdtRec::receive(const Packet &packet)
{
    //首先检查校验和是否正确
    ...
    if (checknum == packet.checksum && this->expectSequenceNumberRcvd == packet.seqnum)
    {
        ...
        lastAckPkt.acknum = packet.seqnum; //确认号等于收到的报文号
        lastAckPkt.checksum = pUtils->calculateCheckSum(lastAckPkt);
